...
namespace AutoScrew
{
    class EleScrew1
    {
        ...
        public static EleScrew1 mTCPIPClient;
        public static EleScrew1 instance()
        {
            if (mTCPIPClient == null) mTCPIPClient = new EleScrew1();
            return mTCPIPClient;
        }
        public EleScrew1()
        {
        ...
        public void TheTCPIPConnect()
        {
            ...
            try
            {
                ...
                string ip = GlobalVar.EleScrew1IP;
                IPAddress ipad = IPAddress.Parse(ip);
                m_ip = new IPEndPoint(ipad, 5000);
                client.Connect(m_ip);
            ...
            try
            {
                ...
                if (client != null && client.Connected)
                {
                    ...
                    if (receiveCount > 0)
                    {
                        ...
                        if (count1 >= 2046)
                        {
                            torq_update1(tempb1, count1);   //接收完原始数据后转到二次处理
                            count1 = 0;
                        }
        ...
        TorqueDataConvert mTorqueDataConvert1 = new TorqueDataConvert();   //处理电批1数据实例对象
        public void torq_update1(byte[] data, int count)   //电批1网络数据
        {
            GlobalVar.originalData1 = new byte[count];
            for (int i = 0; i < count; i++)
            {
                GlobalVar.originalData1[i] = data[i];      //截取原始数据
            }
            //Update_OriginalData1(originalData1);    //保存电批1原始数据

            mTorqueDataConvert1.DataConvert_TCR(data, ref GlobalVar.torq_double_1, ref GlobalVar.Angle_double_1, count, ref GlobalVar.titleAxisY_1, ref GlobalVar.indexSSL1, ref GlobalVar.optID_net_1);   //阅读模式数据解析

            int length = GlobalVar.torq_double_1.Length;
            double[] tempTorque = new double[length];
            double[] tempAngle = new double[length];
            for (int i = 0; i < length; i++)
            {
                tempTorque[i] = Math.Abs(GlobalVar.torq_double_1[i]);  //取扭力绝对值
                tempAngle[i] = Math.Abs(GlobalVar.Angle_double_1[i]);  //取角度绝对值
            }
            GlobalVar.Max_Torque_1 = Math.Round(tempTorque.Max(), 4);  //求最大扭力
            GlobalVar.Max_Angle_1 = Math.Round(tempAngle.Max(), 1);    //求最大角度

            GlobalVar.TKnetDataEnter1 = true;
