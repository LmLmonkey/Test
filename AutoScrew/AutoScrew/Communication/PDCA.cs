...
namespace AutoScrew
{
    class PDCA
    {
        ...
        public static PDCA mTCPIPClient;
        public static PDCA instance()
        {
            if (mTCPIPClient == null) mTCPIPClient = new PDCA();
            return mTCPIPClient;
        }
        public PDCA()
        {
        ...
        void Receive(IAsyncResult ia)
        {
            try
            {
                ...
                GlobalVar.Instance.UpdateRunMessage("PC收到PDCA发来的：" + context, LogName.runLog);
                if (context.Length > 0)
                {
                    lock (SendLock)
                    {
        ...
        public void sendData(string StrData)
        {
            lock (SendLock)
            {
                try
                {
                    ...
                    GlobalVar.Instance.UpdateRunMessage("PC发送：" + StrData + "给PDCA", LogName.runLog);
                }
                catch (Exception e)
                {
                    GlobalVar.Instance.UpdateRunMessage("PC发送向PDCA发送数据失败：" + e.Message, LogName.runLog);
                }
            }
        }
